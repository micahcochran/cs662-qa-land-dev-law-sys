...
q_df = pd.read_csv(f'{file_path}/csv/questions_input.csv', low_memory=False, names=['use', 'x', 'y', 'question'])
...
for index in range(13):
    ...
    with open(f'{file_path}/text/text_{index}.txt', 'r') as file:
        ...
        d_df.to_csv(f'{file_path}/csv/documents{index}.csv', index=False, header=True, lineterminator='\n', columns=['document_identifier', 'document_text'])
